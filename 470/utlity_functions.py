...
def plotmode2d(x,y,v):
    """plots a 2d modeshape v(x,y)
    
    The modal data in v contains the mode shape, and x and y are the spacial coordinates.
    It makes sure a nice colormap, symmetric around 0 is used.
    ...
    lim = max(abs(v))
    # make a contour plot, with nice colormap, ensuring the color-change is at 0
    ax.tricontourf(x,y,v,cmap=plt.get_cmap('RdBu'),vmin=-lim,vmax=lim)
